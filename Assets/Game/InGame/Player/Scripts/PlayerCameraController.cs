...
namespace PPack
{
    public class PlayerCameraController : MonoBehaviour
    {
        ...
        [Tooltip("Camera offsets while in the over-the-shoulder aim mode (SetAimMode(true))")]
        [SerializeField]
        private float _aimCameraDistance = 1.5f;
        [SerializeField]
        private float _aimCameraHeightOffset = 0.2f;
        [SerializeField]
        private float _aimCameraHorizontalOffset = 0.6f;
        [SerializeField]
        private float _aimBlendSpeed = 8f;

        private bool _isAiming;
        private float _currentCameraDistance;
        private float _currentCameraHeightOffset;
        private float _currentCameraHorizontalOffset;
        ...
        private void Start()
        {
            ...
            _currentCameraHeightOffset = _cameraHeightOffset;
            _currentCameraHorizontalOffset = _cameraHorizontalOffset;

            _syntyCamera.localPosition = new Vector3(_currentCameraHorizontalOffset, _currentCameraHeightOffset, _currentCameraDistance * -1);
        ...
        private void Update()
        {
            ...
            float targetDistance = _isAiming ? _aimCameraDistance : _cameraDistance;
            float targetHeightOffset = _isAiming ? _aimCameraHeightOffset : _cameraHeightOffset;
            float targetHorizontalOffset = _isAiming ? _aimCameraHorizontalOffset : _cameraHorizontalOffset;
            float aimBlendT = _aimBlendSpeed * Time.deltaTime;

            _currentCameraDistance = Mathf.Lerp(_currentCameraDistance, targetDistance, aimBlendT);
            _currentCameraHeightOffset = Mathf.Lerp(_currentCameraHeightOffset, targetHeightOffset, aimBlendT);
            _currentCameraHorizontalOffset = Mathf.Lerp(_currentCameraHorizontalOffset, targetHorizontalOffset, aimBlendT);
        ...
        /// <summary>
        ///     Switches the camera between the idle orbit offset and the over-the-shoulder aim offset,
        ///     blending between them over time rather than snapping.
        /// </summary>
        /// <param name="enable">Whether aim mode is active.</param>
