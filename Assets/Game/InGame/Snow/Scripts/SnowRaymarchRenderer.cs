// ---------------------------------------------------------------------------
// AnyTest 스파이크 v7 에서 이식. 원본: Assets/SnowGrainFakeV7/Scripts/SnowRaymarchRendererV7.cs
//
// 필드 타입만 갈아끼웠다 — v7 의 GPU 시뮬(SnowPileFieldV7)이 아니라 우리 SnowDerivedField 가
// 텍스처 넷을 준다. 그 필드는 CPU 권위 격자(SnowField)를 올린 결과이므로, 이 렌더러가 하는 일은
// 원본과 같고 권위는 CPU 에 남는다(루트 AGENTS.md: 데디 서버에는 GPU 가 없다).
//
// 파라미터 이름·기본값·주석은 원본 그대로다. 검증된 튜닝을 다시 유도하지 않는다.
// ---------------------------------------------------------------------------
...
namespace PPack
{
    ...
    public sealed class SnowRaymarchRenderer : MonoBehaviour
    {
        public const string ShaderName = "PPack/SnowRaymarch";
        ...
        [Tooltip("Hard bound on march iterations. A ray that hits it MISSES, so this is a correctness " +
                 ...
                 "READ THE INSTRUMENT BEFORE TOUCHING IT. The [] line prints mean steps, p95 steps and " +
        ...
        [Header("Realistic palette and response - what _SnowCasual = 0 returns to")]
        ...
        // ------------------------------------------------------------------ runtime
        private SnowDerivedField _field;
        ...
        ///    _SnowCasual = 0 A/B is actually comparing to.
        ...
        /// SnowDerivedField.Step, because the bake runs inside the step.
        ///
        ...
        /// </summary>
        public void PushLumpBakeParams(SnowDerivedField field)
        {
        ...
        public void Initialize(SnowDerivedField field)
        {
            _field = field;
            ...
            if (shader == null)
            {
                Debug.LogError("[SnowSpike.Pile] Shader not found: " + ShaderName);
                enabled = false;
                return;
            }

            _material = new Material(shader) { name = "M_SnowRaymarchRuntime" };
            ...
            _mesh = SnowProxyBoxMesh.CreateProxyBox(
                new Vector3(field.PatchSizeX, _maxSnowHeightM, field.PatchSizeZ));

            var go = new GameObject("SnowVolume");
        ...
        public void UpdateUniforms(float dt)
        {
            ...
            _material.SetTexture(kHeightTex, _field.HeightTexture);
            ...
            // zero where they are not. See CoarseMaxYFrom in SnowMarchCore.hlsl for the full argument.
        ...
        /// SnowMarchCore.hlsl is included by this shader AND by the probe kernel, so the code is
        ...
        public bool PushMarchUniforms(ComputeShader cs, int kernel)
        {
            ...
            cs.SetTexture(kernel, kHeightTex, _field.HeightTexture);
