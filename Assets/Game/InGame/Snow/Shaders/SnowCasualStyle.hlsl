// ---------------------------------------------------------------------------
// AnyTest 스파이크 v7 에서 이식. 원본: Assets/SnowGrainFakeV7/Shaders/SnowCasualStyleV7.hlsl
//
...
//    multiplies by curvature AO. Casual snow keeps a little of both - _SnowAlbedoInfluence and
//    _SnowAoInfluence - because zero reads as a sticker, but not the full amount, because the full
...
// ---------------------------------
// _SnowCasual is the master: 0 returns the caller's own realistic colour bit for bit, because
// SnowCasualApply's last act is lerp(realistic, casual, _SnowCasual) and every displacement term is
...
// ------------------------------------------------------------------ uniforms
// Set as GLOBALS by SnowCasualStyle.cs, once per frame, rather than per material. There are four
...
float  _SnowCasual;          // MASTER. 0 = the v3 look bit for bit, 1 = full toy treatment.

// -- banded diffuse ------------------------------------------------------------------------------
float  _SnowBands;           // quantised light steps. 1 + softness 1 is the smooth A/B.
float  _SnowBandSoftness;    // width of each step's transition, in band units. 1 = no plateau.
float  _SnowWrap;            // diffuse wrap before quantisation, so the terminator is not a knife.
...
float4 _SnowLitColor;
float4 _SnowMidColor;
float4 _SnowShadowColor;

float  _SnowAlbedoInfluence; // how much of the caller's albedo survives. 0 = flat palette only.
float  _SnowAoInfluence;     // how much of the caller's AO survives.
float  _SnowExposure;        // overall gain on the casual colour, so the palette can be authored
                            // in the 0..1 range it is easiest to reason about.

// -- rim and sparkle (what replaces the specular) -------------------------------------------------
float  _SnowRimStrength;     // 0 disables. Broad and dim on purpose.
float  _SnowRimPower;        // LOW = broad. High would be a specular by another name.
float4 _SnowRimColor;

float  _SnowSparkleAmount;   // 0 disables.
float  _SnowSparkleScaleM;   // metres per sparkle cell. LARGE, so sparkles are sparse and readable.
float  _SnowSparkleRadius;   // sparkle size in cell units, so it scales with the cell.
float  _SnowSparkleThresh;   // 0..1 fraction of cells that never sparkle at all.
float  _SnowSparkleSpeed;    // twinkles per second. Slow.
float4 _SnowSparkleColor;
float  _SnowTime;            // seconds, pushed from C# rather than read from _Time, because half of
...
float  _SnowRoundM;          // fillet amplitude in metres for the raymarch path. 0 disables.
float  _SnowRoundK;          // headroom in metres at which the fillet is HALF applied. Not a blend width.
...
// SnowCasualApply.
float  _SnowBandNormalWideM;
float  _SnowLoadExaggeration;// NEUTRAL DEFAULT 1.0. >1 makes piled snow look deeper than it is.
float  _SnowLoadLiftMaxM;    // hard bound on that lift, so the raymarcher's coarse bound stays valid.
float  _SnowVirginDepthM;    // the depth exaggeration measures "piled" against.

float  _SnowLumpSquash;      // per-lump vertical squash for the instanced path. 0 = true spheres.
...
// Quantises a 0..1 light term into _SnowBands soft steps.
//
...
{
    float3 lo = _SnowShadowColor.rgb;
    float3 md = _SnowMidColor.rgb;
    float3 hi = _SnowLitColor.rgb;
...
//   * _SnowSparkleThresh silences most cells outright, so the surface is not uniformly glittery;
...
{
    float2 q  = float2(p.x + p.y * 0.6, p.z - p.y * 0.6) / max(0.02, _SnowSparkleScaleM);
    float2 ci = floor(q);
    float2 cf = q - ci;
    ...
    float  blob   = saturate(1.0 - d / max(0.02, _SnowSparkleRadius));

    float twinkle = 0.5 + 0.5 * sin((_SnowTime * max(0.0, _SnowSparkleSpeed) + h2) * 6.2831853);
    float alive   = step(saturate(_SnowSparkleThresh), h2);

    float facing = saturate(dot(normalize(L + V), n));

    return _SnowSparkleColor.rgb
         * (max(0.0, _SnowSparkleAmount) * blob * blob * alive * twinkle * facing * facing);
}

// ------------------------------------------------------------------ the shading entry point

// Turns the caller's realistic ingredients into the toy look, and blends back to the caller's own
// realistic colour by _SnowCasual so a strict A/B is one uniform away.
//
//   realistic  the colour the caller already computed the v3 way. Returned untouched at _SnowCasual 0.
                       ...
                       float shadow, float ao, float3 posWS, float3 lightCol, float3 ambient)
{
    ...
    float wrap = saturate((dot(nBand, L) + _SnowWrap) / (1.0 + _SnowWrap));
    float lit  = SnowCslBand(saturate(wrap * shadow), _SnowBands, _SnowBandSoftness);

    float3 tone = SnowCslTone(lit);

    // Flat-ish: a little of the caller's albedo and a little of its AO, not all of either.
    float3 col = tone * lerp(1.0, albedo, saturate(_SnowAlbedoInfluence));
    col *= lerp(1.0, saturate(ao), saturate(_SnowAoInfluence));
    ...
    col += ambient * _SnowShadowColor.rgb * (1.0 - lit) * 0.25;

    // Broad, dim rim. LOW power on purpose: this is a silhouette read, not a highlight.
    float rim = pow(saturate(1.0 - saturate(dot(n, V))), max(0.5, _SnowRimPower));
    col += _SnowRimColor.rgb * (max(0.0, _SnowRimStrength) * rim);

    col += SnowCslSparkle(posWS, n, L, V) * lightCol;

    col *= max(0.0, _SnowExposure);

    return lerp(realistic, col, saturate(_SnowCasual));
}
...
//
// _SnowRoundK is now the HEADROOM AT WHICH THE FILLET IS HALF APPLIED, in metres, NOT a blend width.
...
{
    float amp = max(0.0, _SnowRoundM) * saturate(thin) * saturate(_SnowCasual);
    liftBoundM = amp;

    float head = max(0.0, coarseH - h);
    float k    = max(1e-4, _SnowRoundK);
...
{
    float k    = max(0.0, _SnowLoadExaggeration - 1.0) * saturate(_SnowCasual);
    float over = max(0.0, h - _SnowVirginDepthM);
    return min(over * k, max(0.0, _SnowLoadLiftMaxM));
}
