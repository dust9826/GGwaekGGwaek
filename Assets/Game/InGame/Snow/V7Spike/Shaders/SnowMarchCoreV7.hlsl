...
// SnowRaymarchV7.shader, because a compute shader has no UNITY_MATRIX_VP and no _MainLightPosition.
...
// HLSL and Unity's inline sampler states only. SnowCasualStyleV7.hlsl is included because the surface
...
// the METAL note in SnowCasualStyleV7.hlsl for the real constraint and why the convention is stricter.
// -----------------------------------------------------------------------------------------------

#include "SnowCasualStyleV7.hlsl"

// THE LUMP LATTICE'S FORMULA AND ITS 8-BIT ENCODING. Included here rather than reproduced, because
// the bake kernel in SnowPileFieldV7.compute includes the same file: the thing that writes the baked
...
// coarse-max build separately - see CoarseMaxBlock in SnowPileFieldV7.compute, which maxes the baked
...
// SnowLumpBakeV7.hlsl, included above, so there is exactly one definition of the surface.
//
...
// onto the field's compute shader by SnowRaymarchRendererV7.PushLumpBakeParams. What is left here is
...
float SurfaceYFromDil(float2 xz, float2 lodFade, float2 dil)
{
    ...
    // over the same dirty window relax uses. See SnowLumpBakeV7.hlsl for the formula and the gates.
    //
