...
//     casual wall top is round, a realistic one is crisp. Its lift is hard clamped to _Cs7RoundM, and
...
//     hard clamped to _Cs7LoadLiftMaxM, and reported the same way. Off by default.
...
//     than a blanket + r over the whole 120 x 110 m field. See CoarseMaxYFrom in SnowMarchCoreV7.hlsl
//     for the upper-bound argument and SnowLumpBakeV7.hlsl for the formula and the encoding.
...
//
// The shading is v3's, computed in full, and then handed to SnowCasualApply. _Cs7Casual = 0 returns the
...
// SnowCasualStyleV7.hlsl for the real failure (an early return that leaves an out parameter
// unwritten), the six live counter-examples in v3, and why a clean single-return audit is
// compliance rather than proof.
Shader "SnowSpike/PileV7/SnowRaymarch"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        // SnowCasualStyleV7.hlsl comes in through it.
        #include "SnowMarchCoreV7.hlsl"
        ...
        half4 ShadeSnow(float3 p, float2 lodFade)
        {
            ...
            // ---- v3's realistic shading, computed in full so _Cs7Casual = 0 reproduces it exactly -----
            ...
            // concentric contours like a topographic map. _Cs7BandNormalWideM 0 passes the shading normal
            // and reproduces that terracing exactly, which is the A/B.
            float3 nBand = (_Cs7BandNormalWideM > 1e-4)
                ? FieldMacroNormal(p.xz, _Cs7BandNormalWideM)
                : n;
        ...
        Pass
        {
            ...
            half4 FragForward(Varyings IN, out float outDepth : SV_Depth) : SV_Target
            {
                ...
                if (hit && _StepsHeatModeV7 < 0.5) col = ShadeSnow(hitWS, lodFade);
                if (hit && _StepsHeatModeV7 >= 0.5)
                {
